...
const PdfSyllabusCategoryManager = () => {
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this category?')) {
      try {
        const response = await fetch(`${API_BASE_URL}/api/pdfsyllabuscategories/${id}`, { method: 'DELETE' });
  ...
  const handleEdit = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/pdfsyllabuscategories/${editingId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div style={styles.container} className="container">
      <div style={styles.header} className="text-center">
        <h2 className="display-6 fw-bold mb-0">PDF Syllabus Category Management</h2>
      </div>
      <div className="p-4">
...
export default PdfSyllabusCategoryManager;