...
const VideoSyllabusRegistration = () => {
  // State management
  ...
  useEffect(() => {
    if (location.state && location.state.selectedSyllabus) {
      ...
      navigate('/videosyllabusdashboard');
    }
  ...
  const registerOrUpdateStudentAfterPayment = async () => {
    try {
      ...
      if (isNewStudent) {
        ...
        const response = await axios.put(`${API_BASE_URL}/api/video-update-student/${existingStudentDetails.studentId}`, formData);
        responseData = response.data;
  ...
  const generateStudentPDF = (studentDetails) => {
    ...
    doc.setFontSize(22);
    doc.text("ARN VIDEO SYLLABUS PURCHASE", 105, 25, { align: 'center' });
    ...
    doc.setFont("helvetica", "bold");
    doc.text(`Access Until: ${formatDate(expiry)}`, 30, 180);
    ...
    const notes = [
      "Once a video syllabus is purchased, it is non-refundable under any circumstances.",
      "With your Student ID, you may purchase any syllabus without completing additional forms.",
      "Access to the video content will be automatically revoked after the expiration date.",
  ...
  const generateInvoicePDF = () => {
    try {
      ...
      pdf.text('ARN Video Syllabus Distribution', 70, 30);
      ...
      if (syllabusTitle.length > 30) {
        ...
        pdf.text(`Video Syllabus: ${firstLine}`, 25, currentY);
        ...
        pdf.text(`Video Syllabus: ${syllabusTitle}`, 25, currentY);
      ...
      currentY += 7;
      pdf.text(`Access Until: ${formatDate(accessExpiration)}`, 25, currentY);
      currentY += 10;
  ...
  const downloadPdfReceipt = (studentDetails) => {
    ...
    doc.save(`ARN_Video_Receipt_${studentDetails.studentId}.pdf`);
  };
  ...
  const processSuccessfulPayment = async (paymentResponse, order) => {
    ...
    try {
      ...
      toast.success('Video syllabus purchased successfully!', {
        position: "top-right",
        autoClose: 3000,
      });
  ...
  const handleSyllabusPurchase = async () => {
    ...
    try {
      ...
      if (isFree) {
        ...
        toast.success('Free video syllabus added to your account!', {
          position: "top-right",
          autoClose: 3000,
        });
              ...
              <Col md={6}>
                <Card className="shadow-lg border-0 rounded-lg">
                  <Card.Header className="bg-info text-white text-center py-4">
                    <h3 className="mb-0">Video Syllabus Registration</h3>
                  </Card.Header>
          ...
          <Container className="mt-5">
            <Row className="justify-content-center">
              <Col md={8}>
                <Card className="shadow-lg border-0 rounded-lg">
                  <Card.Header className="bg-info text-white text-center py-4">
                    <h3 className="mb-0">Video Syllabus Purchase Confirmation</h3>
                  </Card.Header>
                  <Card.Body className="p-4">
                    ...
                    <div className="syllabus-details-container p-3 mb-4 border rounded bg-light">
                      ...
                      <Row className="mb-3">
                        <Col md={6}><strong>Price:</strong> {selectedSyllabus?.fees === 0 ? 'Free' : `INR ${selectedSyllabus?.fees || '0'}`}</Col>
                      ...
                      <Row>
                        <Col>
                          <strong>Description:</strong> {selectedSyllabus?.description || 'Comprehensive video syllabus for exam preparation.'}
                        </Col>
                      </Row>
                    </div>
    ...
    <div>
      ...
      <Modal show={showSyllabusDetailsModal} onHide={() => setShowSyllabusDetailsModal(false)} centered>
        <Modal.Header className="bg-success text-white">
          <Modal.Title>Video Purchase Successful!</Modal.Title>
        </Modal.Header>
        <Modal.Body className="p-4">
          {purchasedStudentDetails && (
            <div>
              ...
              <p><strong>Access Until:</strong> {accessExpiration && formatDate(accessExpiration)}</p>
              ...
              <div className="alert alert-success mt-3">
                <i className="fas fa-check-circle me-2"></i> You can now access this video syllabus from your dashboard.
              </div>
            </div>
          )}
        </Modal.Body>
...
export default VideoSyllabusRegistration;